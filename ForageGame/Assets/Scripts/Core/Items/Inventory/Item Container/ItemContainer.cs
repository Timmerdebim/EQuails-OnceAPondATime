...
namespace Project.Items.Inventory
{
    ...
    {
        public List<InventorySlot> Slots { get; protected set; } = new();
        protected int currentSlotIndex = 0;

        #region Add Functions

        public bool TryAddItemAtIndex(Item item, int slotIndex, int quantity = 1)
        {
        ...
        public bool TryRemoveItemAtIndex(Item item, int slotIndex, int quantity = 1)
        {
            if (!IsSlotValid(slotIndex))
                return false;
