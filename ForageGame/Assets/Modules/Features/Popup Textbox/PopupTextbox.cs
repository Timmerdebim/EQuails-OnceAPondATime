...
public class PopupTextbox : MonoBehaviour
{
    ...
    public void ShowTextbox(bool showTextbox)
    {
        if (showTextbox)
        {
            _textbox.text = _text; // update text right before revealing

            _textbox.color = _color;
            Color.RGBToHSV(_color, out float hue, out float saturation, out float value); //HSV are all 0 to 1
